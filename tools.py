...
class GetWeatherTool(FunctionTool):  # type: ignore[misc]
    ...
    description: str = (
        "获取某地当前、未来约6小时降水、24小时或1至7日天气。用户直接询问现实中的"
        "当前温度、当前天气、是否正在下雨、最近是否下雨或未来天气时，这是明确的"
        "实时事实请求，应调用；不要求用户正在做出行决定。例如“你那今天多少度”或"
        "“你那边最近有下雨吗”。穿衣带伞、通勤旅行、户外活动、晾晒、开窗或日出"
        "日落安排会受天气影响时也调用。文学比喻、虚构场景、天气回忆和纯情绪化表达"
        "（如“心里阴天”）不要调用；仅提到出门、上班或季节也不自动调用。仍由当前"
        "主模型在本轮自主判断，不按关键词强制调用。选择最小够用范围，无法从数据"
        "确认的过去天气要说明限制；调用后不扩展无关环境信息或强行转题。"
    )
    parameters: dict = Field(
        default_factory=lambda: {
            "type": "object",
            "properties": {
                "location": {
                    "type": "string",
                    "description": (
                        "地点名或经度,纬度。用户说“你那”“你那里”“你那边”时，"
                        "仅在单用户部署且已配置有效 default_location 的情况下留空，"
                        "使用插件级全局常驻地点；多用户部署必须显式提供当前用户确认的"
                        "地点。无法确定地点时先自然追问，不得猜测地点、编造天气或等待"
                        "用户自行报告天气。"
                    ),
                    "default": "",
                },
