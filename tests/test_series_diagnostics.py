...
from astrbot_plugin_environment_awareness.series_diagnostics import (
    diagnostic_clear,
    diagnostic_event,
    diagnostic_events,
    logger,
...
def test_series_diagnostics_are_bounded_redacted_and_isolated():
    ...
    diagnostic_event(
        "environment.refresh",
        "refreshed 123456789",
        details={"cached": True, "user_id": "123456789"},
    )
    logger.warning(
    ...
    assert payload["events"][0]["details"] == {"cached": True}
...
def test_series_diagnostics_cursor_and_capacity():
    ...
    for index in range(1005):
        diagnostic_event("environment.event", index)
